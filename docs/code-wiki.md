...
| [agentPlanner.ts](file:///./src/services/llm/agentPlanner.ts) | LLM-first Agent Planner（多动作计划，single/react）；prompt `v1.18` 区分有限批量、整表编排、轮播压缩与“参考已有编单”的对象/维度澄清，并消费可信历史摘要生成增量草案时段。 |
| [trustedForegroundAgentContext.ts](file:///./src/services/runtime/trustedForegroundAgentContext.ts) | Local/Agent Server 的可信前台上下文入口；按当前频道和日期读取 canonical 历史摘要并注入 `referencePlaylists`，不解析用户文本选择历史事实。 |

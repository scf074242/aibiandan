...
## 13. 补充执行卡：参考已有编单的混合旅程

### 13.1 问题

用户说“参考某张已有编单进行编排”时，“已有编单”可能指频道版面结构、某日正式节目内容分布，或连续剧/系列节目的历史播出进度。当前系统已分别具备版面草案引用与历史顺播证据，但不能在参考对象和参考维度不明确时假装读取一张具体正式编单。

### 13.2 期望

1. 未给出可定位日期、工作区或编单身份时，由 LLM 追问参考对象；未说明参考维度时同时追问版面结构、内容分布或顺播进度。
2. 参考版面结构时复用既有版面草案引用；参考顺播进度时复用 canonical 历史编排证据；两者均不得改写当前正式现场。
3. 参考某张正式编单整体重规划时，参考编单只作为证据；先形成当前工作区草案，用户确认后才启动正式 ReAct。
4. 参考事实不存在、不可访问或跨工作区时结构化暴露并保持当前草案、正式播单和 pending 不变。

### 13.3 前置数据

- 当前电视工作区及其正式播单快照。
- canonical 历史编排记录、频道版面草案和其中可追溯到 canonical 节目的条目。
- 用户提供的参考日期/工作区及参考维度；缺失时不得由本地猜测。

### 13.4 风险

- 把“参考”误解为复制并直接覆盖当前正式播单。
- 将版面草案、历史正式编单和顺播进度混为同一数据源。
- 历史事实覆盖当前播单已有节目，或跨 workspace 读取参考现场。
- LLM 只在回复正文声称已读取参考编单，却没有合法 action 或真实 observation。

### 13.5 验证方式

- 在编排员覆盖矩阵与九阶段后矩阵加入五字段完整 case，并由 `agent:check` 门禁覆盖。
- planner 协议验证歧义轮返回 `clarify`，且 prompt 明确参考对象、参考维度、草案审看和正式写入边界。
- 多轮旅程依次验证：参考对象澄清 → owner/证据源确定 → 草案方案 → 用户确认 → 正式 ReAct；只修复黑盒或真实 LLM 实际暴露的问题。

### 13.6 当前验收结果

- case-first 首轮失败确认 `agentPlanner` prompt `v1.13` 没有“参考已有编单”的对象/维度边界；模型行为依赖自由发挥，协议不能证明其一定先澄清。
- prompt 升级为 `v1.14` 后，真实 LLM 在约 13 秒内对“参考东方卫视之前那张已有编单，重新规划今天整张播单”返回 `clarify`，同时追问可定位参考对象与参考维度；没有返回草案、正式编排或原子 mutation。
- 既有“昨天版面草案”切换和“历史进度顺播”继续作为两个已支持分支；参考某张历史正式编单的整体内容分布尚无结构化参考事实入口，因此当前只做到安全澄清，不声称已经读取或可完整复刻。
- 该修复只修改 LLM planner prompt 与版本，不增加本地关键词分类、历史编单复制器、新 capability、子 agent、自动回滚或并行执行路径。

### 13.7 真实命令与贯通目标

真实业务背景：频道临时恢复常规版、节假日复用、同星期版式复用或复盘高收视播单时，编单员通常参考历史正式编单的内容配比，而不是逐条复制旧节目。

多轮命令：

1. 用户：“参考东方卫视之前那张已有编单，重新规划今天整张播单。”
2. Agent：追问具体日期/工作区，以及参考版面结构、节目内容分布还是连续节目进度；当前现场不变。
3. 用户：“参考3月24日东方卫视编单的节目类型配比和收视表现，沿用当前版面，先生成今天的整表草案给我看，不要写正式播单。”
4. 可信运行时读取当前频道可访问的 canonical 历史摘要并放入 planner 上下文；LLM 形成当前工作区草案，不复制历史节目、不写正式播单。
5. 用户审看并明确确认后，继续复用既有 grant → 正式 ReAct → capability → `FormalPlaylistWriteAdapter` → validate 链路。

当前 canonical 边界：2026-03-24 历史记录声明 `itemCount=26`，包含节目类型配比和 `avgRating=8.6`，但只有1条真实节目明细。因此本轮可验证“参考内容配比/收视表现形成草案”，不能声称支持“逐时段复刻整张26条编单”；后者必须暴露明细不足或要求用户提供完整编单。

### 13.8 实际贯通结果

- 可信 Local/Agent Server 上下文按当前 `channelId/date` 读取 canonical 历史摘要，并同时暴露 `itemCount=26` 与 `detailItemCount=1`；前台上传内容不能充当历史事实。
- 两轮状态融合 case `schedulingAgentReferencePlaylistJourney.test.ts` 已通过：第一轮返回 `clarify/noMutation`；第二轮 Planner 返回两个增量草案时段，其中“东方新闻”扩展到 `18:00-19:00`，吸收 canonical 当前版面原有的半小时空档。
- `LayoutDraftService` 未再次调用 LLM，而是把模型明确返回的增量时段确定性合并回其余当前版面；结果仍为19个完整 canonical 版面段，`LayoutDraftValidator` 无错误，草案版本从1更新为2，正式播单快照保持不变。
- 真实 LLM 对模糊首轮已稳定完成澄清；对明确第二轮的多次评估受当前模型服务时延影响出现90秒超时，因此不能声称真实 LLM 端到端稳定完成。超时保持结构化可重试失败，不放宽统一 deadline，也不在后台自动续跑。
- 本次没有新增 capability、子 Agent、自动回滚或并行业务执行路径；只增加可信证据入口、Planner 结构约束、既有草案字段透传和回归门禁。

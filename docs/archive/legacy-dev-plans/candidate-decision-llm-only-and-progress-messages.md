...
本方案已对照以下约束文档逐条核对：[AGENTS.md](file:///./AGENTS.md)、[docs/aibiandan-agent-rules.md](file:///./docs/aibiandan-agent-rules.md)、[docs/agent-development-protocol.md](file:///./docs/agent-development-protocol.md)。
...
**根因**：[atomicCommandCapability.ts#L4571-L4588](file:///./src/services/agent/atomicCommandCapability.ts#L4571-L4588) 的多候选拦截逻辑——`judgeCandidates.length > 1` 时直接返回 `candidateOptions` 进入 `needs_selection`，**跳过**了 4590 行的 `candidateJudge.selectBestCandidate` 调用：
...
**根因**：[candidateJudge.ts](file:///./src/services/agent/candidateJudge.ts) 的 `DefaultAgentCandidateJudge.selectBestCandidate` 用本地关键词匹配打分替代 LLM 决策：
...
**调用与注入点**：
- 调用：仅 [atomicCommandCapability.ts#L4590](file:///./src/services/agent/atomicCommandCapability.ts#L4590) 一处
- 注入：[schedulingAgentRuntime.ts#L70](file:///./src/services/agent/schedulingAgentRuntime.ts#L70) `this.candidateJudge = options.candidateJudge ?? new DefaultAgentCandidateJudge()`
- 接口：[types.ts#L670-L681](file:///./src/services/agent/types.ts#L670-L681)

### 问题 3：缺失"候选决策"独立进度消息

**根因**：[demoRuntimeFacade.ts#L5030-L5103](file:///./src/services/runtime/demoRuntimeFacade.ts#L5030-L5103) 的 `buildAgentTraceProgressEvent` 已有三类进度消息分支，但缺失"候选决策"类：
...
**现状**（[tvSequenceCandidateSelector.ts](file:///./src/services/agent/tvSequenceCandidateSelector.ts)）：作为"前置过滤候选"的选择器，直接返回 candidate 或 candidateOptions。
...
**接口改造**（[types.ts#L670-L681](file:///./src/services/agent/types.ts#L670-L681)）：
...
- [第 6 行](file:///./src/services/agent/schedulingAgentRuntime.ts#L6)：`import { DefaultAgentCandidateJudge } from './candidateJudge'` → `import { LlmAgentCandidateJudge } from './candidateJudge'`
- [第 31 行](file:///./src/services/agent/schedulingAgentRuntime.ts#L31)：`SchedulingAgentRuntimeOptions` 增加 `llmClient?: Pick<LLMClient, 'chat'>` 字段
- [第 70 行](file:///./src/services/agent/schedulingAgentRuntime.ts#L70)：`this.candidateJudge = options.candidateJudge ?? new LlmAgentCandidateJudge({ llmClient: options.llmClient ?? defaultLlmClient })`
...
**修改点**：[L4460-L4618](file:///./src/services/agent/atomicCommandCapability.ts#L4460-L4618)
...
**修改点**：`buildAgentTraceProgressEvent` 函数（[L5030-L5103](file:///./src/services/runtime/demoRuntimeFacade.ts#L5030-L5103)），在"继续查找"分支后新增"候选决策"分支：
...
**同步检查**：`buildAgentAssistantProcessSummary`（[demoRuntimeFacade.ts#L5690 附近](file:///./src/services/runtime/demoRuntimeFacade.ts#L5690)）——确认是否需要同步更新"已筛出 N 个候选"摘要文案，避免与新的"候选决策"进度消息重复。
...
**纳入 `agent:check`**：将新测试文件加入 [package.json#L29](file:///./package.json#L29) `agent:check:tests` 的文件列表。
...
- 若未暴露，调整 [schedulingAgentRuntime.ts#L29-L35](file:///./src/services/agent/schedulingAgentRuntime.ts#L29-L35) 注入

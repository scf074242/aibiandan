...
这份文档只覆盖 [atomic-command-clarification-requirements.md](./docs/atomic-command-clarification-requirements.md) 中的需求二：
...
1. [demoRuntimeFacade.ts](./src/services/runtime/demoRuntimeFacade.ts) 有 `tryContinuePendingAtomicClarification()`，可以把补充输入接回上一轮。
2. [runtimeSessionStore.ts](./src/services/runtime/runtimeSessionStore.ts) 已能保存 `pendingAtomicClarification / pendingTargetSelection / pendingInsertRecommendation`。
3. [openClawBridge.ts](./src/services/openclaw/openClawBridge.ts) 已能在提交、取消、执行成功后维护这些状态。
4. [ChatPanel.vue](./src/components/dialogue/ChatPanel.vue) 已能展示追问态、目标选择态和插入推荐态。
...
当前 [openClawBridge.ts](./src/services/openclaw/openClawBridge.ts) 中的 `resolveStatusFromFeedback()` 仍有一部分依赖文案内容判断状态。

...
这份文档只覆盖 [atomic-command-clarification-requirements.md](./docs/atomic-command-clarification-requirements.md) 中的需求一：
...
1. [paramExtractor.ts](./src/services/paramExtractor.ts) 中的 `extractInsertParams()` 当前要求必须同时抽出 `targetTime + programName`。
2. [demoRuntimeFacade.ts](./src/services/runtime/demoRuntimeFacade.ts) 中的插入分支在拿到候选后，会直接进入 `CandidateSelectionService.selectForInsert()`，最终总是选中一个候选。
3. [candidateSelectionService.ts](./src/services/candidateSelectionService.ts) 当前只有“选一个”的能力，没有“建议多个并等待确认”的状态输出。
4. [runtimeSessionStore.ts](./src/services/runtime/runtimeSessionStore.ts) 当前只有 `pendingAtomicClarification` 和 `pendingTargetSelection`，没有插入候选确认态。
...
建议在 [demoRuntimeFacade.ts](./src/services/runtime/demoRuntimeFacade.ts) 中新增：

...
describe('OpenClawBridge to Orchestrator TV sequence flow', () => {
  ...
  it('确认纯电视频道草案后正式编排会参考昨日进度写入下一集', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '按纯电视频道，9:30到10:15继续播品质剧场：纵有疾风起，接昨天进度顺播，生成版面草案',
      currentSchedule: [],
      gapCount: 1,
      history: [],
    })
  ...
  it('当前版面已有同剧集上下文时会覆盖昨日进度并补中间集', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '按纯电视频道，09:45到10:30继续播品质剧场：纵有疾风起，顺着当前版面补中间集，生成版面草案',
      currentSchedule,
      gapCount: 1,
      history: [],
    })
  ...
  it('确认草案时会用最新节目单上下文阻止早段倒序回填', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '按纯电视频道，08:00到08:45继续播品质剧场：纵有疾风起，顺着排，生成版面草案',
      currentSchedule: [],
      gapCount: 1,
      history: [],
    })
  ...
  it('连续时长多段草案确认后会按段落边界正式编排导视和直播节目', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '14点先来10分钟导视，再50分钟静安寺直播，生成版面草案',
      currentSchedule: [],
      gapCount: 1,
      history: [],
    })
  ...
  it('明确片名在节目库无匹配时确认草案不会硬排相似电视剧', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '14:00到15:00安排生命树电视剧，生成版面草案',
      currentSchedule: [],
      gapCount: 1,
      history: [],
    })
  ...
  it('明确节目标题和集数会作为硬约束贯穿到正式编排', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '09:30到10:15安排节目标题纵有疾风起第5集，生成版面草案',
      currentSchedule: [],
      gapCount: 1,
      history: [],
    })
  ...
  it('所属栏目和节目内容组合关键词会贯穿草案检索选择到正式编排', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '14:00到15:00安排所属栏目看东方、节目内容静安寺的轮播单，内容匹配优先，生成版面草案',
      currentSchedule: [],
      gapCount: 1,
      history: [],
    })
  ...
  it('轮播单收视率优先草案确认后会按收视策略正式编排且不读取昨日顺播', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '14:00到15:00做一版电视剧轮播单，优先选择高收视率节目，生成版面草案',
      currentSchedule: [],
      gapCount: 1,
      history: [],
    })
  ...
  it('轮播单热播优先草案确认后会按话题热度策略正式编排', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '14:00到15:00做一版电视剧轮播单，优先选择当前热播节目，生成版面草案',
      currentSchedule: [],
      gapCount: 1,
      history: [],
    })
  ...
  it('正式编排会保留目标时段内已有节目并只填剩余空窗', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '14:00到15:00安排静安寺外场直播轮播单，内容匹配优先，生成版面草案',
      currentSchedule,
      gapCount: 1,
      history: [],
    })
  ...
  it('目标时段中间已有节目时会按结构化关键词只填前后剩余空窗', async () => {
    ...
    const prepare = await bridge.submitInstruction({
      ...
      channelName: '东方卫视',
      date,
      text: '14:00到15:00安排所属栏目看东方、节目内容静安寺的轮播单，内容匹配优先，保留已有节目，生成版面草案',
      currentSchedule,
      gapCount: 2,
      history: [],
    })

...
const editorDemandCoverageCaseDrafts: EditorDemandCoverageCase[] = [
  ...
  {
    id: 'tv-reference-existing-playlist-clarifies-evidence-scope',
    category: 'formal_orchestration',
    playlistModel: 'tv',
    userRequest: '参考东方卫视之前那张已有编单，重新规划今天整张播单',
    expectedDisposition: '先确认具体参考日期或工作区，并问清是参考版面结构、节目内容分布还是连续节目进度；参考事实可定位后先生成草案供审看。',
    supportLevel: 'guarded_supported',
    harness: 'llm_protocol',
    coveredBy: ['agentPlanner.promptVersion.test.ts'],
    rootCauses: ['missing_user_input', 'business_guardrail'],
    reverseInference: '“已有编单”可能指版面模板、历史正式节目表或顺播基线；三者的数据入口和后续动作不同，必须由 LLM 澄清，不能由本地关键词猜测。',
    userInput: '参考东方卫视之前那张已有编单，重新规划今天整张播单',
    expectedDecision: '返回 clarify，追问参考编单身份和参考维度；补齐事实后先准备/调整草案，确认后进入正式 ReAct。',
    mustNotHappen: '猜测历史日期；直接复制旧正式编单；让历史覆盖当前现场；跨工作区读取；同轮正式写入。',
    verification: 'planner 协议回归断言参考对象、参考维度、草案审看和 noMutation 边界。',
  },
  {
    id: 'tv-reference-existing-playlist-builds-draft-from-trusted-history',
    category: 'layout_draft',
    playlistModel: 'tv',
    userRequest: '参考3月24日东方卫视编单的节目类型配比和收视表现，沿用当前版面，先生成今天的整表草案给我看，不要写正式播单',
    expectedDisposition: '读取当前频道可访问的历史编单摘要，由LLM结合当前版面生成今天的整表草案；历史只作证据，正式播单保持不变。',
    supportLevel: 'guarded_supported',
    harness: 'llm_protocol',
    coveredBy: ['foregroundAgentContextPackage.test.ts', 'agentPlanner.promptVersion.test.ts', 'schedulingAgentReferencePlaylistJourney.test.ts', 'schedulingAgentRuntime.realLlmEvaluation.test.ts'],
    rootCauses: ['context_management_gap', 'business_guardrail'],
    reverseInference: '真实编单员常在临时改版、节假日复用或同星期版式复用时参考历史单的内容比例；可信运行时提供事实，LLM决定如何形成草案，本地不能选历史单或复制节目。',
    userInput: '参考3月24日东方卫视编单的节目类型配比和收视表现，沿用当前版面，先生成今天的整表草案给我看，不要写正式播单',
    expectedDecision: '基于结构化历史摘要返回 prepare/refine_layout_draft；回复明确当前只是草案，正式现场 noMutation。',
    mustNotHappen: '用用户文本本地解析参考日期；将历史摘要伪装成完整节目明细；直接复制旧节目；启动 formal_write。',
    verification: '可信上下文、planner prompt、两轮状态融合 journey 和真实 LLM 评估共同断言历史证据可见且动作仅属于草案。',
  },
  {
  ...
  },
  {
    ...
    category: 'composite_task',
    playlistModel: 'rotation',
    userRequest: '分析当前3小时轮播单，按热播优先形成压缩到2小时的方案',
    expectedDisposition: '先读取当前正式轮播单并查证热播依据，research observation 回到 LLM，由 LLM 生成完整2小时压缩草案供用户审看。',
    supportLevel: 'guarded_supported',
    harness: 'foreground_runtime',
    coveredBy: ['schedulingAgentReactTaskRuntime.test.ts'],
    rootCauses: ['context_management_gap'],
    reverseInference: '整表内容取舍需要结合当前节目事实和候选证据；本地候选排序只能提供 observation，不能替编排员生成完整压缩方案。',
    userInput: '分析当前3小时轮播单，按热播优先形成压缩到2小时的方案',
    expectedDecision: 'research_check 后由 LLM 返回覆盖00:00:00-02:00:00的 refine_layout_draft，展示草案且 noFormalPlaylistWrite=true。',
    mustNotHappen: '要求用户指定某一草案段；本地按 topCandidates 自动拼草案；同轮删除或改写正式节目。',
    verification: '前台运行时黑盒注入 canonical 候选 observation，断言第二次 LLM 决策生成完整2小时草案且正式现场不变。',
  },
  {
    id: 'rotation-compress-confirmed-draft-enters-formal-react',
    category: 'formal_orchestration',
    playlistModel: 'rotation',
    userRequest: '这个2小时压缩草案可以，按它开始正式编排',
    expectedDisposition: '确认草案后进入正式 ReAct，先检索和候选裁决，再以 grant 约束批量原子动作，经 FormalPlaylistWriteAdapter 写入并校验总时长。',
    supportLevel: 'guarded_supported',
    harness: 'contract_test',
    coveredBy: ['demoRuntimeFacade.fullGenerateBootstrap.test.ts', 'formalOrchestrationRuntime.test.ts', 'formalPlaylistWriteAdapter.test.ts'],
    rootCauses: ['business_guardrail'],
    reverseInference: '已有正式节目整表重编必须先获得任务级授权；草案确认只开启正式链路，不能绕过候选裁决、mutationPolicy 或写入边界。',
    userInput: '这个2小时压缩草案可以，按它开始正式编排',
    expectedDecision: '签发绑定现场版本与草案指纹的 grant 后执行 research → decide → atomic mutation → validate，最终总时长为7200秒。',
    mustNotHappen: '把草案确认当成已完成；无 grant 写入；跳过 observation 后的 LLM decide；失败时自动回滚。',
    verification: '正式长流程门禁断言 grant、capability、FormalPlaylistWriteAdapter 与最终 validate 的先后顺序。',
  },
  {

...
export class AtomicCommandCapability implements AgentCapability {
  ...
  private async selectCandidate(
    ...
    // 候选查询完成：用于前端"查节目库"气泡展示
    // candidateCount = 进入 LLM 决策的候选数（judgeCandidates，无可播放候选时回退到 rawJudgeCandidates）
    // playableCandidateCount = 已通过硬约束的可播放候选数
    // 注意：此处 noMutation=true，仅记录事实，不影响后续 LLM 决策
    // keyword 在调用方 trace（"调用节目查询服务查找候选"）已记录，此处不重复
    runtime.trace.record('planning', '候选查询完成', {
      commandIntent,
      candidateCount: judgeCandidates.length,
      playableCandidateCount: playableJudgeCandidates.length,
      targetTime,
      noMutation: true,
    })
